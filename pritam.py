...
def meal_NoMeal_Data_Extract(file_path_insulin, file_path_cgm):
    df_insulin = pd.read_excel(file_path_insulin, parse_dates=[['Date', 'Time']],
    ...
    while (k >= 0):
        ...
        if (df_cgm.at[meal_instances[j], 'Date_Time'] - df_cgm.at[
            meal_instances[k], 'Date_Time']).total_seconds() / 60 ** 2 >= 2:
        ...
        if (df_cgm.at[meal_instances[j], 'Date_Time'] - df_cgm.at[
            meal_instances[k], 'Date_Time']).total_seconds() / 60 ** 2 >= 4:
            index_start = meal_instances[k] - 1
            p = index_start
            while (p):
        ...
        if (df_cgm.at[meal_instances[j], 'Date_Time'] - df_cgm.at[
            meal_instances[k], 'Date_Time']).total_seconds() / 60 ** 2 >= 6:
            index_start = meal_instances[k] - 1
            p = index_start
            while (p):
        ...
        if (df_cgm.at[meal_instances[j], 'Date_Time'] - df_cgm.at[
            meal_instances[k], 'Date_Time']).total_seconds() / 60 ** 2 >= 8:
            index_start = meal_instances[k] - 1
            p = index_start
            while (p):
        ...
        if (df_cgm.at[meal_instances[j], 'Date_Time'] - df_cgm.at[
            meal_instances[k], 'Date_Time']).total_seconds() / 60 ** 2 >= 10:
            index_start = meal_instances[k] - 1
            p = index_start
            while (p):
        ...
        if (df_cgm.at[meal_instances[j], 'Date_Time'] - df_cgm.at[
            meal_instances[k], 'Date_Time']).total_seconds() / 60 ** 2 >= 12:
            index_start = meal_instances[k] - 1
            p = index_start
            while (p):
    ...
    while (k >= 0):
        ...
        for each in range(0, 30):
            if (k > 0):
                ...
                if ((df_cgm_meal.at[index_start, 'Date_Time'] - df_cgm_meal.at[
    ...
    while (k >= 0):
        ...
        for each in range(0, 24):
            if (k > 0):
                ...
                if ((df_cgm_no_meal.at[index_start, 'Date_Time'] - df_cgm_no_meal.at[
                    next_index, 'Date_Time']).total_seconds() / 60 <= 5):
...
pd.DataFrame(zip(cross_validation_results['test_score'], cross_validation_results['train_score']),
             columns=['TestScore', 'TrainScore'])
...
model = svm.SVC(kernel='linear', C=3,
                gamma=1)  # svc for classification: kernel linear performance of C=3 is best so far
...
clf = MLPClassifier(solver='lbfgs', alpha=0.01, hidden_layer_sizes=(5, 4), random_state=1, learning_rate='adaptive',
                    activation='tanh')
clf.fit(X_pt1, Y_pt1)
